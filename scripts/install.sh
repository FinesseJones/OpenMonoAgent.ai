...
case "$OPENMONO_ROLE" in
    ...
    inference)
        ...
        echo "  mode              : $([ "${HAS_GPU:-false}" = true ] && echo GPU || echo CPU)"
        echo ""
        echo "This machine is now the inference server."
        echo ""
        echo "Next steps:"
        echo "  1. source ~/.bashrc"
        echo "  2. On THIS inference box, connect it to a relay server:"
        echo "       openmono tunnel setup --inference"
        echo "     This will print two commands — copy them and run them on the agent box."
        echo "  3. On the AGENT BOX, paste the commands from step 2, then run:"
        echo "       openmono agent"
        echo ""
        ;;
    agent)
