...
EOF
    ...
    info "Writing CPU/Vulkan override: $OVERRIDE_FILE"
    ...
    if [ -d /dev/dri ] && ls /dev/dri/renderD* &>/dev/null 2>&1; then
        HAS_VULKAN_GPU=true
        # Resolve group IDs needed for DRM device access
        VK_GROUPS="44"    # video
        _RENDER_GID=$(stat -c "%g" /dev/dri/renderD128 2>/dev/null || echo "")
        [ -n "$_RENDER_GID" ] && VK_GROUPS="$VK_GROUPS, $_RENDER_GID"
        info "Vulkan GPU detected via /dev/dri — enabling GPU passthrough"
    fi

    # Vulkan device passthrough block (indent-sensitive)
    VK_DEVICES=""
    VK_GROUP_ADD=""
    if [ "$HAS_VULKAN_GPU" = true ]; then
        VK_DEVICES='    devices:
      - /dev/dri:/dev/dri'
        VK_GROUP_ADD="    group_add:
      - \"44\"    # video group
      - \"${_RENDER_GID:-991}\"   # render group"
    fi
...
OVERRIDE_EOF
    ok "CPU/Vulkan override written"
